...
var syncronizedDelay = 10 * 60 * 1000; //10 minutes
var streamTimer;
var data = {};
var options = {};
var chart;
...
$(document).ready(function ()
{
    ...
    $.notify({ message: 'This chart is designed to collect data over Days!<br>...Watch the soil dry.'}, { type: 'success', delay: 12000 });
...
function buildGraphMenu() {
    ...
    var e_settings = $("<i>", { class: "icons icon-settings h1 m-4", onClick: "$(\".graphSettings\").trigger(\"click\");", "data-toggle": "tooltip", "title": "Settings" });
    var e_img = $("<i>", { class: "icons icon-png h1 m-4", onClick: "exportPNG()", "data-toggle": "tooltip", "title": "Export Image" });
    var e_csv = $("<i>", { class: "icons icon-csv h1 m-4", onClick: "exportCSV()", "data-toggle": "tooltip", "title": "Export CSV" });
    ...
    input_speed.ionRangeSlider({
        ...
        min: 1,
        max: 120,
        from: (syncronizedDelay / 60 / 1000),
        prettify: speed_prettify,
        postfix: " Minutes",
        onFinish: function (e) {

            if(btn_start[0].disabled == true) {
                $.ajax("usb.php?eeprom=write&offset=" + ee + "," + e_deepSleep + "&value=" + parseInt(0xEF) + "," + (e.from*8).toFixed(0), { async: true});
            }
            ...
            syncronizedDelay = e.from * 60 * 1000; //1000 = second, 1000000 = minute
...
function startChart() {
    ...
    $.ajax("usb.php?eeprom=write&offset=" + ee + "," + e_deepSleep + "&value=" + parseInt(0xEF) + "," + ((syncronizedDelay/60/1000)*8).toFixed(0), { async: true});
    
    updateChart(idDatasets(chart_datasets));
};
...
function updateChart() {
	...
	xhr = $.ajax("usb.php?eeprom=read", {
        ...
        success: function(response)
        {
            ...
            var h2o = (ref * 100) / 1.8 //scientific value (actual H2O inside soil)
            console.log(adc);
            ...
            streamTimer = setTimeout(function() {
                updateChart();
            }, syncronizedDelay + 1000); //wait exra for eeprom to write
        },
        error: function() {
